...
__kernel void batch_norm_k(__global float* inout, __global float* beta, __global float* scaled_gamma, __global float* mean, __global float* var, int HW, int C)
{
    int i = get_global_id(0);
    int c = get_global_id(1);
    float scaled_gamma;
    //scaled_gamma = gamma[c] / (float)sqrt(var[c] + 1e-5);
    inout[i * C + c] = scaled_gamma * inout[i * C + c] + (beta[c] - scaled_gamma[c] * mean[c]);
}
